...
Page({
  ...
  onSelectBatchItem(e) {
      ...
      const batchItems = this.data.batchList.filter(b => b.batch_number === selectedBatchNo);
  ...
  onAmountInput(e) { this.setData({ withdrawAmount: e.detail }); },

  // 用途选择处理
  onUsageClick() { this.setData({ showUsagePicker: true }); },
  ...
  async onSelectAggregatedItem(e) {
      ...
      try {
          ...
          const batches = res.data.map(b => {
              ...
              const rawDate = b.expiry_date || (b.dynamic_attrs && b.dynamic_attrs.expiry_date);
