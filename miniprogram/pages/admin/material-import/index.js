...
const MATERIAL_TEMPLATE_HEADER_ROWS = [
  ['产品代码', '物料名称', '类别', '子类别', '默认单位', '化材包装形式', '膜材厚度(μm)', '默认幅宽(mm)', '供应商', '原厂型号', '是否测试料'],
  ['必填', '必填', '必填', '必填', '必填', '化材选填', '膜材必填', '膜材选填', '选填', '选填', '选填']
];
...
Page({
  ...
  onCopyTemplateStructure() {
    ...
    const templateRows = [
      ...
      // 固定提示行
      ['必填', '必填', '必填', '必填', '必填', '必填', '化材选填', '膜材必填', '膜材选填', '选填', '选填', '选填'],
      // 空行 - 从这里开始填写数据
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ...
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['▶ 字段说明：（* 表示必填）', '', '', '', '', '', '', '', '', ''],
      ['代码前缀*：必填，例如 J-、S-、Y-、M-，请以系统当前启用前缀为准', '', '', '', '', '', '', '', '', ''],
      ...
      ['模板仅用于新建物料；若完整产品代码已存在，系统会跳过，不会更新现有主数据', '', '', '', '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['▶ 当前有效子类别：', '', '', '', '', '', '', '', '', ''],
