...
const MATERIAL_TEMPLATE_HEADER_ROWS = [
  ['是否测试料', '类别', '代码前缀', '产品编号', '物料名称', '子类别', '默认单位', '化材包装形式', '膜材厚度(μm)', '默认幅宽(mm)', '供应商', '原厂型号'],
  ['是/否，空白=否', '必填', '必填', '必填', '必填', '必填', '必填', '化材选填', '膜材必填', '膜材选填', '选填', '正式料选填/测试料必填']
];
...
Page({
  ...
  onCopyTemplateStructure() {
    ...
    // 模板内容（用 Tab 分隔便于粘贴到 Excel）
      ...
      ['类别*：必填，只能填 "化材" 或 "膜材"', '', '', '', '', '', '', '', '', ''],
      ['代码前缀*：必填。请先选择本行类别，再选择该类别可用前缀；只填写 1-4 位英文字母，不填写横杠', '', '', '', '', '', '', '', '', ''],
      ...
      ['是', '化材', 'J', '999', '环氧树脂样品', '树脂', 'g', '瓶装', '', '', '供应商A', 'TEST-RESIN-A'],
      ...
      success: (res) => {
        try {
          ...
          const dataRows = rows
            ...
            .filter(item => item.values.length >= 5 && item.values[1] && item.values[2] && item.values[3] && item.values[4]);
