...
**方案**：以后端为主 —— 区分「技术异常」与「业务失败」，仅对成功结果建立可复用回执；业务失败不写入或写入后允许覆盖。前端在业务失败后清除 `operation_id` 作为兜底（`material-add/index.js:1916`）。

**注意**：改动须保证幂等的原有目的不被削弱 —— 重复提交同一笔成功操作仍必须返回首次结果，不能重复扣减。
