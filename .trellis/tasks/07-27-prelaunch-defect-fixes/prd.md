...
- [x] `npm test` 全部通过。**最终 582/582**（基线 569，净增 13 条覆盖用例）
- [x] `npm run preflight:deploy` 通过
- [x] 全项目无新增的事务内 `where(...).update()/remove()/count()` 用法（已用精确匹配复查，违规 0 处）
- [x] 共享文件改动后 `npm run sync:shared` 执行成功且副本一致
- [ ] 领用 `2.01` / `1.001` / `32.3` 不再报「库存不足」 ← 待真机
- [ ] 测试料按批次领料可正常完成 ← 待真机
- [ ] 物料申请**连续审批两条**均成功 ← 待真机（只审一条验不出 B4）
- [ ] 单件入库后，标签详情页可见「初始录入」日志 ← 待真机
- [ ] 同一产品代码下两个原厂型号的测试料，项目用料汇总显示为两行 ← 待真机
- [ ] 修改测试料原厂型号 / 产品代码后，该测试料仍可正常入库 ← 待真机
- [ ] 已过期批次的徽标显示「已过期」而非「即将过期」，且仍可领用 ← 待真机

> 代码侧验收已全部通过；带「待真机」的七项依赖测试环境端到端走查，清单见 `implement.md` 的 Post-delivery 章节。
...
| 提交 | 内容 |
|---|---|
| `bf2643a` | 阶段一：四个阻断项（B1 / B2 / B3+B4） |
| `460cf42` | 阶段二：日志字段补全（F1 / F2） |
| `9c83e09` | 阶段三：测试料主数据级联（H3 / H4 / H5） |
| `9c1a97a` | 阶段四：事务重试与幂等（H7 / H6 / H11） |
| `56d87ee` | 阶段五：P1 四项（H1 / H2 / 过期显示 / H8） |
| `80b9265` | 计划外：补 H3 的库存保护，修正自己引入的回归 |
| `8999004` | 计划外：操作编号时效 |
| `184eb6c` `ea71046` | 计划外：物料目录空状态文案与排版 |

### 三处方案偏离（与 design.md 原方案不同）

1. **H6 / H11 改用前端方案**：初版改 `_shared/operation-receipts.js` 让 `failed` 不再复用，撞上 `tests/operation-receipts.test.js` 中「failed responses are terminal and reusable」的明确断言 —— 那是有意设计（保证同一次提交的重复请求返回一致结果），改动会影响全部 9 个使用幂等回执的云函数。撤回后改为前端在业务失败时清除 `operation_id`，效果相同且不触碰后端语义。

2. **H5 改为归档而非阻断**：原定「存在型号则阻断批量删除」，实现时改为「有型号则归档」—— 与既有的「有库存历史则归档」走同一路径，既保住型号关联又不必打断整批操作。

3. **H1 采用方案 B**：明确不用哨兵日期（`expiry_date` 与 `is_long_term_valid` 五处互斥校验），改为读满候选再统一排序。同时把「读满扫描上限」的处理从「一律抛错」改为「基于完整候选集判断」—— 否则恰好读满上限且够量的合法场景会被误判为「库存范围过大」。

### 计划外的三项追加

1. **H3 的库存保护**（`80b9265`）：仅重算 key 会让历史库存与型号库失配、那批货领不出来 —— 用一个同样严重的问题换掉了唯一性绕过。详见 `design.md` D6。

2. **操作编号时效**（`8999004`）：外部复核提出的残余风险，核实后发现比描述更顽固 —— 编号缓存在持久化 storage 里，刷新和重启都清不掉，用户只能靠改内容或清缓存自救。加 30 分钟时效即可自愈，且不削弱短时重试的幂等保护。

3. **物料目录空状态**（`184eb6c` `ea71046`）：原文案只提「先维护测试料型号」，漏掉正式料这条路径；修正后又发现折成两行且断在动宾结构中间 —— 根因是 vant 的 `padding: 0 60px` 叠加全局 20px，375px 屏幕上仅剩 215px 可用。文案与间距一并调整后单行展示。
